...
//Modificado em 25/05/2026, o backup está antes deste script
using UnityEngine;
...
public class PointCloudSelector : MonoBehaviour
{
    ...
    [Header("Referência da Garra Fantasma")]
    public GhostGripperManager ghostManager; // <-- NOVA REFERÊNCIA AQUI
    ...
    public void ExecuteSelection()
    {
        ...
        if (foundValidPoint)
        {
            ...
            if (targetIndicator != null)
            {
                ...
                if (hideIndicatorCoroutine != null) StopCoroutine(hideIndicatorCoroutine);
                hideIndicatorCoroutine = StartCoroutine(HideIndicatorAfterDelay());
            }

            // EXIBE A GARRA FANTASMA (O GGCNN Subscriber continuará movendo ela)
            // EXIBE A GARRA FANTASMA APENAS QUANDO ELA CHEGAR NO ALVO
            if (ghostManager != null) ghostManager.ShowGhost(selectedTargetPoint);
        ...
        else
        {
            ...
            // ESCONDE A GARRA SE APONTAR PARA O VAZIO
            if (ghostManager != null) ghostManager.HideGhostInstantly();
        }
    }
